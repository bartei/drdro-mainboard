...
# nmcli is the Linux NetworkManager CLI; it isn't present on Windows/macOS. Guard every call on
# it so the Network screen degrades to a read-only "unavailable" notice instead of crashing app
# startup on non-Linux hosts (target platforms include Windows/macOS for development).
...
class NetworkScreen(Screen):
    ...
    def __init__(self, **kv):
        ...
        if not NMCLI_AVAILABLE:
            log.warning("nmcli not found — network management is unavailable on this platform")
    ...
    async def connect(self):
        if not NMCLI_AVAILABLE:
            self.log("Network management unavailable on this platform (nmcli not found)")
            return
